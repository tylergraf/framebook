...
class CBApp extends LitElement {
  ...
  _locationChanged() {
    ...
    this._page = path[0] === '' ? 'categories' : path[0];
    if(path[1]){
      this._id = path[1];
    }
    ...
    switch (this._page) {
      case 'categories':
        Promise.resolve().then(function () { return cbCategories; });
        break;
      case 'category':
        Promise.resolve().then(function () { return cbSubcategories; });
        break;
      case 'subcategory':
        Promise.resolve().then(function () { return cbRecipes; });
        break;
      case 'recipe':
        import('./cb-recipe.js');
        break;
...
var cbCategories = /*#__PURE__*/Object.freeze({

});

// import './cb-header.js';
const service$1 = new CookbookService('http://localhost:3000');
...
class CBSubcategories extends LitElement {
  ...
  async _init(id){
    this._subcategories = await service$1.subcategories(id);
  }
  set id(val){
    if(!val) return;
    this._init(val);
...
var cbSubcategories = /*#__PURE__*/Object.freeze({

});

// import './cb-header.js';
const service$2 = new CookbookService('http://localhost:3000');
...
class CBRecipes extends LitElement {
  ...
  async _init(id){
    this._recipes = await service$2.recipes(id);
  }
  set id(val){
    if(!val) return;
    this._init(val);
    ...
    return html`
      ...
      <ul>
        ${this._recipes && this._recipes.map(recipe=>html`
          <li>
            <a href="/recipeegory/${recipe._id}">${recipe.title}${recipe.subtitle}</a>
          </li>
        `)}
      </ul>
    `
...
var cbRecipes = /*#__PURE__*/Object.freeze({
